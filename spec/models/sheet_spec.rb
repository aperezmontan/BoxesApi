...
describe ::Sheet do
  ...
  let(:sheet_params) { { :name => "new_sheet", :home_team => "NY", :away_team => "NJ", :user_id => user.id, :game_id => game.id } }
    ...
    context "when sheet doesn't have all the correct params" do
      ...
      context "errors" do
        context "when Sheet is created with invalid params" do
          let(:invalid_params) { { :name => "new_sheet", :home_team => "NY", :user_id => user.id, :game_id => game.id } }
